function statsTbl = makepolarplots_V3(y, t, madMult)
...
if isempty(theta1_f) || isempty(theta2_f)
    ...
    theta1_f = theta1;
    theta2_f = theta2;
    keep1 = true(size(theta1));
    keep2 = true(size(theta2));
    usedFiltered = false;
end

% =======================
% STATS (computed on what you plot)
% =======================
...
std_max_h = circStd(theta1_f) * (24/(2*pi));
std_min_h = circStd(theta2_f) * (24/(2*pi));

% Circular MAD (hours): median absolute circular deviation from circular median
mad_max_h = median(circAbsDevHours(theta1_f, theta_med_max), 'omitnan');
mad_min_h = median(circAbsDevHours(theta2_f, theta_med_min), 'omitnan');
...
nMax = numel(theta1);
nMin = numel(theta2);
nMaxKept = numel(theta1_f);
nMinKept = numel(theta2_f);

statsTbl = table( ...
    ["Max"; "Min"], ...
    [mean_max_h; mean_min_h], ...
    string({meanMax_str; meanMin_str}), ...
    [med_max_h;  med_min_h], ...
    string({medMax_str;  medMin_str}), ...
    [std_max_h;  std_min_h], ...
    [mad_max_h;  mad_min_h], ...
    [nMax; nMin], ...
    [nMaxKept; nMinKept], ...
    [nMax-nMaxKept; nMin-nMinKept], ...
    repmat(madMult,2,1), ...
    repmat(usedFiltered,2,1), ...
    'VariableNames', { ...
        'Extrema', ...
        'MeanHour','MeanHHMM', ...
        'MedianHour','MedianHHMM', ...
        'StdHour','MADHour', ...
        'N_raw','N_kept','N_removed', ...
        'MADmult','UsedFiltered'});
...
title(pax, sprintf('Extrema by time of day (±%.1f MAD filtered)', madMult))
...
% --- Label mean times (from FILTERED) ---
