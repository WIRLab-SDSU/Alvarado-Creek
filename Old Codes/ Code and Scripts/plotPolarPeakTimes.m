...
function [max_mean_hour,min_mean_hour, max_stD, min_stD] =  plotPolarPeakTimes(param, time_vec,bin_width)

% --- Peaks (maxima and minima) ---
[pks_1, locs_1] = findpeaks(param, time_vec, 'MinPeakDistance', 0.75);
[pks_2, locs_2] = findpeaks(-param, time_vec, 'MinPeakDistance', 0.75);
...
pax.RTick = [];
pax.ThetaTickLabelMode = "manual";
hold(pax,'on')

% --- Choose binning in HOURS, then convert to radians ---
binWidthHours = bin_width;
...
title(pax,'Peak times by time of day')
...
% Store the mean peak times in the output variables
max_mean_hour = hourmeanMax_str;
min_mean_hour = hourmeanMin_str;

% Calculate standard deviations for maxima and minima
max_stD = std(h1);
min_stD = std(h2);
